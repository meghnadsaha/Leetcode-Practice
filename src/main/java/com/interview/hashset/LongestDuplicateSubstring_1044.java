...
public class LongestDuplicateSubstring_1044 {
	...
	private class CompressedString {
		...
		CompressedString(int li, int ri) {
			leftIncl = li;
			rightIncl = ri;
		}
		...
		// Derive the sequence-dependent rolling hash signature of the
		// substring.
		@Override
		public int hashCode() {
	...
	private void precomputations() {
		...
		// Compute base-26 sum of every substring from the beginning to end
		// (idx).
